...
export class AuthService {
  ...
  getToken(): string | null {
    //récupérer le token TOKEN_KEY à partir de localStorage
   ...
   */
  isLoggedIn(): boolean {
    //récupérer le token à partir de getToken()
    const token = "";
  ...
  private setAuthData(user: User, token: string): void {
    // Sauvegarder dans localStorage TOKEN_KEY

    //Sauvegarder dans localStorage USER_KEY
   ...
   */
  logout(): void {
    // Supprimer les données du localStorage TOKEN_KEY

    // Supprimer les données du localStorage USER_KEY


    // Notifier l'observable currentUserSubject

     // Notifier l'observable isAuthenticatedSubject
